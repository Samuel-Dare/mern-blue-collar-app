...
// import { urlGetServiceProviders } from '../../services/apiProfessionals';
import { useTaskInfoContext } from '../../context/TaskInfoContext';
import { BASE_URL } from '../../config/config';
import { useNavigate } from 'react-router-dom';
import { useUser } from '../authentication/useUser';
import { useProfessionals } from './useProfessional';
...
const ServiceProviderProfileCard = () => {
  ...
  const {
    taskInfo: { selectedService, date, time, price, sortBy },
  } = useTaskInfoContext();
  ...
  const handleSelectProfessional = () => {
    if (!date) return toast.error('Please choose your preferred date');
    if (!time) return toast.error('Please choose your preferred time');

    if (!isAuthenticated) return navigate('/signup-login');
  ...
  if (data && data.length > 0) {
    return (
      <main className="md:min-w-2/3 space-y-5 ">
        {data.map((d, i) => (
          <div
            key={i}
          ...
          >
            <div className="space-y-2 text-center md:space-y-3">
              ...
              <div className="hidden md:block">
                ...
                <Button onClick={handleSelectProfessional} type="primary">
                  Select and Continue
                </Button>
